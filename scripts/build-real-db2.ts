...
async function main() {
  ...
  for (const [id,token,url,emoji,label,ca] of stickers) await tx.execute({sql:'INSERT INTO stickers (id,token,url,emoji,label,created_at) VALUES (?,?,?,?,?,?)',args:[id,token,url,emoji,label,ts(ca)]});
  ...
  for (const [id,pid,uid,ih,ca] of likes) await tx.execute({sql:'INSERT INTO likes (id,paste_id,user_id,ip_hash,created_at) VALUES (?,?,?,?,?)',args:[id,pid,uid,ih,ts(ca)]});
